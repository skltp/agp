 ...
 */
abstract class TP02SmokeAbstract extends Simulation {
  ...
  def smoke(serviceName:String, urn:String, responseElement:String, responseItem:String) = scenario("smoke")
                 .during(testDuration) {
                   exec(session => {
   ...
   def setUpAbstract(serviceName:String, urn:String, responseElement:String, responseItem:String, baseUrl:String) : PopulatedScenarioBuilder = {
     smoke(serviceName, urn, responseElement, responseItem).inject(rampUsers(numberOfConcurrentUsers) over (rampDuration)).protocols(http.baseURL(baseUrl).disableResponseChunksDiscarding) 
   }
}
